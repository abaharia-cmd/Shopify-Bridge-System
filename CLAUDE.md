...
## Current Phase
**Live inventory_levels sync + processor self-heal landed** (2026-06-11). `inventory_items.incremental(id)` added; `inventory_levels/update` webhooks now route through it (reads `inventory_item_id` from payload, builds GID, calls `runIncremental`). Re-fetches the item + all levels (~800ms per webhook). Dropped `inventory_levels.location_id_fkey` (Shopify references virtual fulfillment-service locations the regular `locations(...)` query doesn't return — same partial-mirror reality as fulfillment_orders). Added `processing_started_at` + `last_failed_at` columns + partial indexes on `shopify_sync.webhook_events`; processor now stamps `processing_started_at` on claim and at the top of every invocation resets rows stuck in 'processing' for >10 min back to 'received' (handles Vercel timeout / OOM / crashes). Verified live: 189 levels rows / 5 min, 0 FK errors, queue draining at ~50 events/min.
...
### 2026-06-11 — `shopify_sync.webhook_events.last_failed_at` column was missing — `markFailed` had been a silent no-op
[src/worker/webhookProcessor.ts](src/worker/webhookProcessor.ts) `markFailed()` writes `last_failed_at`, and `claimNextEvent()` reads it via `.lt("last_failed_at", cutoff)`. But the column didn't exist in the table — Phase 3B added the code without adding the column. PostgREST UPDATEs to unknown columns just error and the whole update reverts, so any time a webhook handler threw, the row was supposed to go to status='failed' but actually stayed at status='processing' forever. The "Vercel timeout" stuck-row theory absorbed this — both look identical in the data (rows in 'processing' with last_error=null because the markFailed UPDATE didn't take). Migration `webhook_events_processing_tracking` added the column. Lesson: when code writes to a new column, sanity-check `information_schema.columns` before deploying — Supabase JS doesn't validate schema at compile time, and the error gets logged but execution continues.

### 2026-06-11 — Self-healing for stuck 'processing' rows: stamp on claim + reset on next invocation
Added `processing_started_at TIMESTAMPTZ` (partial index where status='processing'). `claimNextEvent` now sets it when flipping a row from received→processing. Top of every `processWebhookQueue` invocation runs `resetStuckProcessingRows()` — two-pass UPDATE: (1) `status='processing' AND processing_started_at < now - 10min`, (2) `status='processing' AND processing_started_at IS NULL AND received_at < now - 10min` (legacy rows from before the migration). Cheap, indexed, idempotent. Vercel function maxDuration is 300s; 10 min as the cutoff means no false positives on live in-flight work. Removes the need for manual `UPDATE ... SET status='received' WHERE status='processing'` resets every few weeks.

### 2026-06-11 — inventory_levels live sync: route inventory_levels/update via inventory_items.incremental
Payload only carries `available` — committed/incoming/on_hand/etc. are NOT in the webhook. Don't try to UPDATE in place from the payload; re-fetch the full item via the new `inventory_items.incremental(id)` which mirrors the bulk QUERY shape (with `inventoryLevels(first: 50)`). Receiver extracts no resource_id for this topic (the inventory_levels/update payload has no top-level `id`); the processor reads `payload.inventory_item_id` directly and constructs `gid://shopify/InventoryItem/<id>`. ~800ms per webhook = ~70 webhooks/min via the existing concurrency model — fits the per-minute cron + receiver fire-and-forget pattern.

### 2026-06-11 — inventory_levels.location_id FK dropped: virtual Shopify locations
Real Shopify referenced 2 locations (`83696025779`, `83696091315`) that don't appear in our `locations` table. Both are likely fulfillment-service / drop-ship virtual locations (Shopify's `locations(...)` query only returns physical ones). Every inventory_levels/update for an item with stock at one of these locations was failing with `inventory_levels_location_id_fkey` violation — silently logged to error_log but never marked failed on the webhook itself (parent inventory_items upsert succeeded, child failure landed in per-batch isolation). Dropped the FK; kept the column + index. Same partial-mirror pattern as fulfillment_orders.location_id from 2026-05-12. Pattern lesson: any FK from a live-sync child table to a parent we backfill ONCE will eventually break this way. Future incremental-sync targets (returns, transactions, etc.) should default to no cross-table FKs and rely on the natural-key composite UNIQUE for idempotency.
